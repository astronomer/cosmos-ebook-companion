...
def cosmos_airflow_features_snowflake():
    ...
    @task 
    def depend_on_staging_model():
        pass

    _depend_on_staging_model = depend_on_staging_model() 
    _depend_on_staging_model.set_upstream(dbt_project.get_child_by_label("model1_run"))

    chain(pre_dbt(), dbt_project, post_dbt())


cosmos_airflow_features_snowflake()
